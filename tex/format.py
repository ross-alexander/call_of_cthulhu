...
def format_base(character, template_tbl):
...
def format_stats(character, template_tbl):
    ...
    if not 'LUCK' in stats:
        ...
        stats['LUCK'] = luck
    template_tbl['stats']['LUCK'] = stats['LUCK']
...
def format_skills(character, template_tbl):
    skill_table = {
        ...
        'Stealth': { 'base': 20},
        'Swim': { 'base': 20},
        'Throw': { 'category': 'Fighting', 'base': 20},
        'Track': { 'base': 10},
    }
    ...
    if not 'skills' in character:
        print("Skills: allocate the following values to eight skills and Credit Rating [%s]" % ",".join([str(i) for i in skills_allowed]))
        exit(1)
    ...
    # --------------------
    # Map skills with subskill to category
    # --------------------

    skills_convert = {}

    for skill in sorted(skills_char.keys()):
        value = skills_char[skill]
        if skill in category_table:
            for subskill, value in value.items():
                if not isinstance(value, dict):
                    value = { 'cur': value }
                    skills_convert[subskill] = value
                value['category'] = skill
        else:
            if not isinstance(value, dict):
                value = {'cur': value}
                skills_convert[skill] = value

    skills_char = skills_convert

    # --------------------
    # Check values
    # --------------------

    if not 'updated' in character:
        cnt_allowed = Counter()
        cnt_diff = Counter()
        for i in skills_allowed:
            cnt_allowed[i] += 1
            cnt_diff[i] += 1
            cnt_values = Counter()
        for i in skills_char.values():
            print(i)
            cnt_values[i['cur']] += 1

        cnt_diff.subtract(cnt_values)
        if +cnt_diff:
            print("Skills don't match allowed values: %s (%s vs %s)" % (cnt_diff, cnt_allowed, cnt_values))
            exit(1)
    ...
    for name in sorted(skills_char.keys()):
    ...
    # --------------------
    # Split base skills over two columns with third for subskills
    # --------------------
...
def format_character(opts):
    ...
    res = template.render(character = template_tbl)
